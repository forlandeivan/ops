...
export const JANITOR_TASKS: readonly JanitorTaskDefinition[] = [
  ...
  // Старые наборы фрагментов: до 23.09.2026 переиндексация оставляла прежний набор в таблице навсегда
  // (на проде судов — 44 % фрагментов). Теперь вытесненные наборы удаляет сама задача индексации, а
  // политика убирает накопленное до обновления и остатки после сбоев. Отбор в ops свой
  // (tasks/pg-retention-task.ts): возраст строки не говорит, нужен ли набор. Пачка — во фрагментах,
  // а не в наборах: набор тома-скана бывает на десятки тысяч строк. Фильтр is_latest описывает то же
  // условие для гейта индексов. Выключена из коробки: первую уборку большой установки запускают вручную.
  task({
    key: "pg.knowledge_document_chunk_sets.superseded",
    label: "Старые версии фрагментов баз знаний",
    description:
      "Удаляет наборы фрагментов, оставшиеся от прежних индексаций документов: поиск и ответы их не читают. Набор текущей индексации и набор индексации, которая идёт сейчас, не удаляются. Размер пачки считается во фрагментах. Освободившееся место база использует заново; чтобы вернуть его на диск, таблицу сжимают отдельно.",
    category: "knowledge",
    action: "delete_rows",
    ...
    intervalMinutes: 30,
    sensitive: true,
    cascadeNote: "Фрагменты удаляемых наборов уходят каскадом; векторы в Qdrant не трогаются.",
  }),
