...
export const COLLECTION_FIELD_TYPES = [
  "string",
  // Legacy/worker-compatible types from indexing policy
  "double",
  "object",
  // Extended semantic types used by wizard config
  "integer",
  "float",
  "boolean",
  "text",
...
export interface StartIndexingWithConfigRequest {
  mode: "full" | "changed" | "outdated" | "errors";
...
// ============================================================================
// Per-base override индексационных настроек
// Хранится в knowledge_bases.indexing_config_override (jsonb).
// Совпадает по форме с подмножеством KnowledgeBaseIndexingPolicyDto, чтобы
// override прозрачно подменял глобальную политику для конкретной базы.
// ============================================================================

export const knowledgeBaseIndexingOverrideSchema = z
  .object({
    chunkSize: z.number().int().min(200).max(8000),
    chunkOverlap: z.number().int().min(0),
    defaultSchema: z.array(
      z.object({
        name: z.string().trim().min(1).max(120),
        type: z.enum(["string", "double", "object"]),
        isArray: z.boolean(),
        template: z.string(),
      }),
    ).max(50),
...
export type KnowledgeBaseIndexingOverride = z.infer<typeof knowledgeBaseIndexingOverrideSchema>;
