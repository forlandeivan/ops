...
/**
 * Метаданные выполнения RAG-пайплайна.
 * Новые multi-query поля опциональны для обратной совместимости со старыми ответами API.
 */
export interface KnowledgeBaseRagMetadata {
  totalResults?: number;
  ...
  rrfApplied?: boolean;
  rrfInputDocuments?: number;
  rrfOutputDocuments?: number;
  rrfConstant?: number;
  rrfTopN?: number;
}

/**
 * Метаданные чанка для результатов RRF fusion.
 * Поля опциональны, поэтому single-query ответы остаются совместимыми.
 */
export interface KnowledgeBaseRagChunkMetadata {
  rrfScore?: number;
  appearancesCount?: number;
  bestOriginalScore?: number;
...
export function mergeRagSearchSettings(
  ...
  const result: KnowledgeBaseRagSearchSettingsResolved = {
    ...
    maxCompletionTokens: constraints.maxCompletionTokens.defaultValue,
  ...
  if (input) {
    ...
    if (Object.prototype.hasOwnProperty.call(input, "maxCompletionTokens") || Object.prototype.hasOwnProperty.call(input, "maxTokens")) {
      const parsed = parseNumber((input as Record<string, unknown>).maxCompletionTokens ?? (input as Record<string, unknown>).maxTokens);
      result.maxCompletionTokens = parsed === null
        ? null
        : clampInteger(parsed, constraints.maxCompletionTokens.min, constraints.maxCompletionTokens.max);
    }
