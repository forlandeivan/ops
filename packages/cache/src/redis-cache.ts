 ...
 * Done as a pre-pass (faster than JSON.stringify replacer on every value).
 * Uses Number() since our BigInt columns hold counters/sizes well within
 * Number.MAX_SAFE_INTEGER (9 PB for byte counters, 9 quadrillion for tokens).
 */
...
export class RedisCache implements CacheProvider {
  ...
  constructor(redisUrl: string, options?: { keyPrefix?: string; connectTimeout?: number }) {
    ...
    this.client.on('connect', () => {
      logger.debug('Redis cache connecting');
    });
  ...
  async get<T>(key: string): Promise<T | null> {
    try {
      ...
      logger.error({ key, err: error }, 'Redis cache get error');
  ...
  async set(key: string, value: unknown, ttl?: number): Promise<void> {
    try {
      const fullKey = this.getKey(key);
      const serialized = JSON.stringify(sanitizeBigInts(value));
      ...
      logger.error({ key, err: error }, 'Redis cache set error');
  ...
  async del(key: string): Promise<void> {
    try {
      ...
      logger.error({ key, err: error }, 'Redis cache del error');
    }
  }
  ...
  async delByPrefix(prefix: string): Promise<void> {
    try {
      ...
      logger.error({ prefix, err: error }, 'Redis cache prefix delete error');
    }
  }
  ...
  async clear(): Promise<void> {
    try {
      ...
      logger.debug({ count: keys.length }, 'Redis cache cleared');
    } catch (error) {
